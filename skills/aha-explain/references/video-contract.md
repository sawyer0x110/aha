...
| `voice` | Explicit identifier; Edge uses a locale-prefixed Neural voice such as `zh-CN-XiaoxiaoNeural`. For imported speech use a stable alphanumeric/hyphen label such as `user-recording`. |
...
Online synthesis requires the installed Python `edge-tts==7.2.8` adapter plus FFmpeg/ffprobe. Missing or incompatible dependencies are explicit blockers, not authorization to install or switch providers. Only the reviewed segment text, voice, and rate go to Microsoft speech processing.
