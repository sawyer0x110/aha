...
For this repository's gallery, the manifest uses `schemaVersion: 2`, `layout: "topic-format-v1"` and `requestedOutputs[]`. Each topic is published at `examples/<topic>/`, with sources at `projects/<format>/`, a sealed `research/`, and QA at `qa/<format>/`. Only `examples/index.html` is the gallery. `evals/examples/` contains reusable checking tools, not publication archives. The gallery verifier checks the exact requested inventory and format-specific evidence; it is not a general CLI validator for arbitrary user task manifests.
