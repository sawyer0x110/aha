...
- **Claim:** required `id`, `text`, `evidenceIds`, `limitations`; optional `kind` (`"fact"`, `"inference"`, `"unresolved"`) and `subquestionIds`. Ordinary claims need evidence. An unresolved claim needs explicit limitations and linked gaps through the covering subquestion's `gapIds`; claims have no `gapIds` field. An unresolved claim cannot answer a subquestion. `subquestions[].claimIds` is the primary coverage map: every claim must be linked from a subquestion. `claim.subquestionIds` may be omitted; when supplied, it must list exactly all covering subquestion IDs, in any order, not a subset. An explicit empty reverse list contradicts nonempty primary coverage. During draft checking, an unfilled primary list can remain pending; contradictory populated links are errors. Strict sealing requires both directions to agree.
