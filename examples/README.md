...
- [`../evals/examples/`](../evals/examples/)：当前运行结果、分别记录的文字与图形审阅、代表性截图；与作品目录分开，不代表真人验收。
...
`experiment-recipe.json` 保留执行路径及失败说明，是实验方法记录，不是当前目录的可执行脚本；结论以 `experiment-results-v2.json` 为依据。`project/qa/author-review.json` 记录源码交接时的审阅范围，最终浏览器状态以 `../evals/examples/` 为准。

当前检查和审阅见 [运行记录](../evals/examples/runtime.json)与[审阅记录](../evals/examples/review.json)。文字已分别按中文、英文审读，再核对事实；页面检查覆盖两种语言、浅深主题、1280/390px 宽度和键盘交互。真人理解效果、独立人工翻译验收、手机真机及读屏器检查仍未完成。
...
该命令检查两个当前 `index.html`，将运行结果写入仓库根目录下的 `evals/examples/runtime.json`；`--screenshots` 可选，会更新该目录的代表性截图。复现的新文件先另存，确认后再替换当前作品及配套收据。
