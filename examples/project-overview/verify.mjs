...
for (const [format, filename] of [['html', 'index.html'], ['image', 'overview.png'], ['pptx', 'overview.pptx']]) {
  ...
  if (format === 'pptx') {
    ...
    const illustration = await zip.file('ppt/slides/slide6.xml').async('string');
    assert.match(illustration, /It is separate from the narrated ANC and Git walkthrough/);
    assert.doesNotMatch(illustration, /Video source supplies both scenes/);
...
assert.equal(correction.researchHash, research.manifest.contentHash);
assert.equal(correction.otherSlidesAndMediaUnchanged, true);
