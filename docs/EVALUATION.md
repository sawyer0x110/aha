...
当前作品为 `examples/git-merge/git-merge.html` 与 `examples/docker-layers/docker-layers.mp4`，由[统一交付清单](../examples/delivery-manifest.json)绑定各自 `qa/<format>/` 中的观察、画面、授权及限制。交付不保留旧对照结果或迁移记录，也不将一般收录认可算作听审或理解验收。Docker 辅助文字在 640px 播放宽度下仍偏小，实际听审与真人理解未验证。
