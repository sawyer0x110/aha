...
`evals/task-video/evals.json` 单独保存两项基于现有 `examples/` 研究的试测：Git 合并假设方案的中文 HTML 审查，以及 Docker 镜像层的英文机制视频试片。它不改变四个固定案例及其 lock，也不把旧成品作为新技能的结果。Git 案例只审查封存材料支持的假设方案，不冒充当前仓库或新实验；Docker 案例固定现有三段旁白，仅比较新的视觉创作，不评价旁白写作能力。
...
首轮试测后的定向修订补充了互斥状态标签的替换方式、持续上下文与辅助文字的取舍，以及方案审查论证链的窄屏重排。这些指导不在首轮冻结候选中，不能沿用首轮结果宣称已改善。后续对照需另冻技能并另存运行；重点观察过渡中间帧、带字幕的实际播放尺寸，以及窄屏下同一假设与证据／建议的联系。横向滚动的二维表格不自动判失败，也不为提高可读性删除必要限定。

2026-09-22 第二轮以首轮候选为基线、三项细则版为候选，重新生成四份作品并获批离线渲染。原十项判据为基线 9/10、候选 10/10；本轮观察到候选窄屏论证纵向排列、视频互斥标签原位切换，而视频辅助文字在小窗口下仍偏小，密度仅部分改善。源帧几何诊断不能替代像素审阅：相邻两行文字的边界框相交不一定是可见重影。每条件仅一次、评分非盲；HTML 候选另加载了宿主主题技能，存在不对称干预，不能作 Aha-only 因果结论。完整解码、静音播放和录音身份核验不等于实际听审或真人理解，两者仍未执行。

用户审阅后批准把第二轮候选收录为 `examples/git-merge/index.html` 与 `examples/docker-layers/pilot.mp4`，其他格式不变。源码、收据、原录音及新的计划关联一并保留，[本轮收录记录](../evals/examples/task-video-20260922/publication.json)封存当前页面／视频观察、有限像素证据、评分摘要和技能身份。按用户要求清理首轮与第二轮临时运行目录，包括基线成品、重复输出、冻结 bundle 和临时 viewer；摘要中的原路径仅为历史定位，不再保证可打开或完整重放原对照。保留证据不是把旧结果改名为新结果，也不把一般收录认可算作听审或理解验收。
