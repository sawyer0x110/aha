...
当前作品是五个主题各四种格式，共二十份；其中 ANC、格陵兰、CPython、Docker 的十六份已重新创作，Aha 项目介绍保持不变。由[统一交付清单](../examples/delivery-manifest.json)绑定各自 `qa/<format>/` 中的观察、画面、授权及限制。这批成品不是上述两个试测的受控对照结果。交付不保留旧对照结果或迁移记录；小播放器的辅助文字、实际听审与真人理解仍有明确限制。
