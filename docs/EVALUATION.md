...
目录按用途分开：[`evals/skills/`](../evals/skills/) 保存技能对照用例与判据；[`evals/examples/`](../evals/examples/) 保存可复用检查工具；[`examples/`](../examples/) 保存作品、可编辑项目和各主题 `qa/<format>/` 中的当前证据。成品验收不是技能对照结果。`npm run examples:verify` 检查当前作品的身份一致性，不代替视觉、听审或读者理解评价；具体覆盖范围与未验证项以各份证据为准。
...
`evals/task-video/evals.json` 单独保存两项基于现有 `examples/` 研究的试测：Docker 清理假设方案的中文 HTML 审查，以及镜像层英文机制视频。它不改变四个固定案例及其 lock，也不把旧成品作为新技能的结果。方案审查只依据封存材料，不冒充实际构建；视频使用当前计划全部五段旁白，按录音实测时长制作，不强压到 20–30 秒，仅比较视觉创作而不评价旁白写作能力。

先构建并冻结修改前后的完整 `aha-explain` bundle，再运行准备器：

```powershell
node evals\task-video\prepare.mjs "<new absolute run root>" "<absolute baseline skill>" "<absolute candidate skill>"
```

准备器拒绝已有输出目录，记录技能全部文件及输入的字节哈希，核对两条件的运行时、资源、schema 和依赖相同。作者只读取自己的 `task.json`、冻结 skill 和 `inputs/`，向 `outputs/` 新建项目；不会收到旧 HTML／视频／镜头源码、评分答案或另一条件的结果。`evals.json` 与每案例 `eval_metadata.json` 只供评价者读取。目录隔离不是 OS 沙箱，没有完整工具追踪时不得认证所有过程限制。

每案例每条件先各运行一次独立作者，记录实际可得的模型／耗时／token 信息，缺失项保持未知。作者可使用受信任 CLI 静态检查、初始化并打包 HTML，但不执行页面、不导入或合成语音。主持人先检查新源码，再单独取得本地执行批准；视频还需展示完整未改动的旁白、新 `provided-audio` 计划哈希与录音来源，获得当前计划的导入批准。复用旧录音不是新的 TTS，不能修改哈希绕过新源码绑定。

内容断言、浏览器交互、完整页面、视频关键状态／边界及实际观看收听分别记录。视频段内按比例安排的节拍不是词级对齐；冻结旁白也不代表新画面的节奏正确。修订结果另存，保留首次输出与失败，不把主持人的修稿记成新 skill 的首次收益。用 skill-creator stock viewer 展示两条件作品和逐项证据；真人理解未收集就保持未验证，单次试测不能证明稳定提升。运行产物保存在忽略的 `artifacts/skill-evals/`，不自动替换 `examples/`。

对照重点观察互斥状态标签的替换、持续上下文与辅助文字的取舍、过渡中间帧、带字幕的实际播放尺寸，以及窄屏下同一假设与证据／建议的联系。横向滚动的二维表格不自动判失败，也不为提高可读性删除必要限定。源帧边界框相交不一定是可见重影，几何诊断不能替代像素审阅；单次非盲试测不能证明稳定收益，宿主主题等不对称干预需另行记录。

当前作品覆盖五个主题、四种格式，共二十一份；Aha 项目介绍包含重构后的英文、中文视频，其余十九份成品保持不变。由[统一交付清单](../examples/delivery-manifest.json)绑定各自 `qa/<format>/` 中的观察、画面、授权及限制，视频语言版本再分 `en/`、`zh/`。新版视频研究与未改动格式的研究分别封存。这批成品不是上述两个试测的受控对照结果。交付不保留旧对照结果或迁移记录；小播放器的辅助文字、实际听审与真人理解仍有明确限制。

### 原生 PPTX 创作规则对照

`evals/pptx-first-round/` 单独评估逐页设计意图、中文容量修复、对象／图片选择和备注分工，不修改上述四个固定案例及其 lock。它使用 `examples/` 已封存的 CPython 字符串、ANC、格陵兰面积与投影及 `aha-introduction` 研究。项目介绍案例采用 2026-09-17 综合研究，解释两技能的宿主协调及格陵兰、CPython、Docker 三个机制；另提供一张保留的 Git 历史幻灯片截图检验图片与原生解释的分工，不将旧作品数量或格式清单当成当前事实。已有示例 PPTX 和作者源码不发给生成者，防止把复刻成品当成技能效果。
