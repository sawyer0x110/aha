...
Use Node.js 22+ and the committed npm lockfile in a trusted source checkout. End users should install the release package rather than build the repository; see [INSTALL.md](INSTALL.md).
...
Keep English and Chinese entry guides synchronized in the same change: `README`, `INSTALL`, `CONTRIBUTING`, `SECURITY`, `docs/USAGE`, and `examples/README`. File names use `.zh-CN.md` for the Chinese peer. Translate meaning, constraints, commands, and links, not just headings; label linked Chinese-only advanced guides. The English `skills/**` contracts remain a single source of truth, not duplicated translated skill bundles.

The root installation guides are copied into new release packages. Keep their relative links limited to files shipped beside them, and use repository URLs for source-only documents. If changing release contents, update the builder, installer allowlist, asset checksums, workflow, and tests together. Do not overwrite a published version or move its tag; use a new release version when publishing changed packages.
