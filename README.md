...
**截至 2026-09-18，`v0.3.1` 尚未发布，GitHub 最新 Release 为 `v0.3.0`。** 当前仓库已准备新版本；本地构建和打包不等于对外发布。需要这些修复时，可按[安装指南](INSTALL.md#发布尚不可用时授权后从源码构建)经授权从源码构建；正式发布仍需单独确认 tag 与上传时机。
