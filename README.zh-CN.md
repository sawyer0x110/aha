...
[English](README.md) | [简体中文](README.zh-CN.md) · [安装](INSTALL.zh-CN.md) · [使用指南](docs/USAGE.zh-CN.md) · [示例](examples/README.zh-CN.md)
...
从 [GitHub Releases](https://github.com/sawyer0x110/aha/releases) 下载完整包，按[安装指南](INSTALL.zh-CN.md)（[English](INSTALL.md)）操作。需要 **Node.js 22+**；接收方**无需 `npm install`**。统一安装到项目 `.agents\skills`，再单独确认宿主能发现、运行两个 Skill。源码 `skills` 目录不是可直接安装的包。
...
源码／包 runtime 为 **0.3.1**，研究／作品 Schema 为 **1.0.0**。已发布 `v0.3.1` 包属于历史发布；本次文档与打包调整待未来发布，不宣称新版本已发布。安装时以所选 Release 实际附带的资产为准。

## 文档与贡献

| 入门 | 深入参考 |
| --- | --- |
| [安装](INSTALL.zh-CN.md) / [English](INSTALL.md) | [产品需求](docs/PRD.md)（中文） |
| [使用与 CLI 工作流](docs/USAGE.zh-CN.md) / [English](docs/USAGE.md) | [架构与目录导航](docs/ARCHITECTURE.md)（中文） |
| [参与贡献](CONTRIBUTING.zh-CN.md) / [English](CONTRIBUTING.md) | [研究协议](docs/RESEARCH.md)（中文） |
| [安全政策](SECURITY.zh-CN.md) / [English](SECURITY.md) | [评估协议](docs/EVALUATION.md)（中文） |

项目原创代码与文档采用 [MIT 许可](LICENSE)。第三方依赖、引用材料与保留示例的边界见[许可范围](LICENSE-SCOPE.md)；收录不等于对外部材料统一重新授权。进阶人类文档和历史证据保留原有语言。
